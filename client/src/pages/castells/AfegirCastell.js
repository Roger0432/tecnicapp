...
function AfegirCastell({ assaig }) {
  ...
  const esborrarCastell = (id) => {
    setSelectedCastells(
      selectedCastells.filter((castell) => castell.id !== id)
    );
  };
    ...
    <Box m={2} mt={4} display="flex" justifyContent="center">
      ...
      >
        <Typography variant="body1" mb={1}>
          {assaig
            ? "Selecciona les proves que vols afegir"
            : "Selecciona els castells que vols afegir"}
        </Typography>
        ...
        <Autocomplete
          ...
          onChange={(event, newValue) => {
            if (newValue) {
              const selectedCastell = castells.find(
                (castell) => castell.id === newValue.id
              );
              if (
                selectedCastell &&
                !selectedCastells.includes(selectedCastell)
              ) {
