...
function authenticateToken(req, res, next) {
  ...
  if (!token)
    return res
      .status(401)
      .json({ msg: "Token no proporcionat", status: false });

  jwt.verify(token, JWT_SECRET, (err, user) => {
    if (err)
      return res.status(403).json({ msg: "Token no vàlid", status: false });
    req.user = user;
    next();
  });
}
...
app.post("/inicisessio", async (req, res) => {
  ...
  try {
    ...
    if (!(await bcrypt.compare(password, user_db.password))) {
      return res
        .status(401)
        .json({ msg: "Contrasenya incorrecte", status: false });
    }
...
app.post("/crear-esdeveniment", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ msg: "Esdeveniment creat correctament", id: id, status: true });
  } catch (error) {
    console.error("Error: ", error);
    res.status(500).json({ msg: "Error del servidor", status: false });
  } finally {
...
app.get("/detalls-esdeveniment/:id", async (req, res) => {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .json({ msg: "Esdeveniment no trobat", status: false });
    }
...
app.post("/editar-descripcio-castell", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ msg: "Descripció actualitzada correctament", status: true });
  } catch (error) {
    console.error("Error: ", error);
    res.status(500).json({ msg: "Error del servidor", status: false });
  } finally {
...
app.delete("/borrar-esdeveniment/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ msg: "Esdeveniment eliminat correctament", status: true });
  } catch (error) {
    console.error("Error: ", error);
    res.status(500).json({ msg: "Error del servidor", status: false });
  } finally {
...
app.put("/editar-esdeveniment/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ msg: "Esdeveniment actualitzat correctament", status: true });
  } catch (error) {
    console.error("Error: ", error);
    res.status(500).json({ msg: "Error del servidor", status: false });
  } finally {
...
app.get("/proxims-esdeveniments", async (req, res) => {
  ...
  try {
    ...
    if (resultAssaig.rows.length === 0 && resultDiada.rows.length === 0) {
      return res
        .status(404)
        .json({ msg: "No hi ha esdeveniments", status: false });
    }
...
app.put("/editar-membre/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ msg: "Membre actualitzat correctament", status: true });
  } catch (error) {
    console.error("Error: ", error);
    res.status(500).json({ msg: "Error del servidor", status: false });
  } finally {
...
app.post("/guardar-castells", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ msg: "Castells guardats correctament", status: true });
  } catch (error) {
    await client.query("ROLLBACK");
    console.error("Error: ", error);
    res.status(500).json({ msg: "Error del servidor", status: false });
...
app.delete("/borrar-castell/:id", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ msg: "Castell eliminat correctament", status: true });
  } catch (error) {
    console.error("Error: ", error);
    res.status(500).json({ msg: "Error del servidor", status: false });
  } finally {
